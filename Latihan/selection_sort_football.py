def selection_sort_football(player) :
    n = len(player)
    for i in range (n) :
        max_idx = i 
        for j in range (i+1,n) :
            if player[j]["jumlah-gol"] > player[max_idx]["jumlah_gol"] :
                max_idx = j
        player[i], player[max_idx] = player[max_idx], player[i]
...
selection_sort_football(daftar_pemain)
